    ...
    try:
        ...
        try:
            first_event = await asyncio.wait_for(
                stream.__anext__(), timeout=10
            )
        except asyncio.TimeoutError as exc:
            raise RuntimeError("Camera track produced no frame for 10s") from exc

        frame = first_event.frame
        width, height = int(frame.width), int(frame.height)
        rotation = int(first_event.rotation)
        ...
        video_filter = rotation_filter(rotation)
        if video_filter:
        ...
        async def write_event(event) -> None:
            nonlocal frame_count, last_timestamp_us
            current = event.frame
            if int(current.width) != width or int(current.height) != height:
                raise RuntimeError(
                    f"Resolution changed: {width}x{height} -> "
                    f"{current.width}x{current.height}"
                )
            process.stdin.write(bytes(current.data))
        ...
        LOG.info(
            "Recording %ss: %sx%s, %.2f FPS, rotation=%s",
            seconds, width, height, fps, rotation,
        )
