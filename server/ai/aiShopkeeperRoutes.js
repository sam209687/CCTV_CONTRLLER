...
function registerAiShopkeeperRoutes(
  ...
  // PATCH_11J_B1_ZONE_HELPERS
  const aiProjectRoot = path.resolve(
    __dirname,
    "..",
    "..",
  );

  const aiConfigurationFile = path.resolve(
    process.env.CCTV_AI_CONFIG_FILE ||
      path.join(
        aiProjectRoot,
        "config",
        "ai-shopkeeper.json",
  ...
  const aiRuntimeRoot = path.resolve(
    process.env.CCTV_RUNTIME_DIR ||
      path.join(
        aiProjectRoot,
        "runtime",
      ),
  ...
  function normalizedAiZone(
    value,
    fallback,
  ) {
    return validAiZone(value)
      ? value.map(
          (item) =>
            Number(
  ...
  async function readAiJson(filePath) {
    try {
      return JSON.parse(
        await fsp.readFile(
          filePath,
  ...
  async function readAiConfiguration() {
    const document =
      await readAiJson(
        aiConfigurationFile,
      );
  ...
  async function writeAiConfiguration(
    document,
  ) {
    await fsp.mkdir(
      path.dirname(
  ...
  function serializeAiConfiguration(
    cameraId,
    stored,
  ) {
    const customerZone =
  ...
  async function getCameraAiConfiguration(
    cameraId,
  ) {
    const document =
      await readAiConfiguration();

    return serializeAiConfiguration(
      cameraId,
      document.cameras[cameraId],
    );
  }

  async function saveCameraAiConfiguration(
    cameraId,
    customerZone,
    staffZone,
  ) {
    ...
    async (request, response) => {
      try {
        ...
        response.json({
          ok: true,
          config:
            await getCameraAiConfiguration(
              cameraId,
            ),
        });
      } catch (error) {
    ...
    async (request, response) => {
      try {
        ...
        if (
          !cameraId ||
          !validAiZone(
            request.body?.customerZone,
          ) ||
          !validAiZone(
            request.body?.staffZone,
          )
        ) {
          response.status(400).json({
            ok: false,
            error:
              "Valid cameraId, customerZone and staffZone are required",
          });
          return;
        }

        const configuration =
          await saveCameraAiConfiguration(
            cameraId,
            request.body.customerZone,
            request.body.staffZone,
          );
        ...
        const [
          ...
          readAiJson(
            path.join(
              aiRuntimeRoot,
              "cctv-recorder",
              "continuous_recorder_status.json",
            ),
          ),
        ]);
